...
pub struct Signature {
    ...
    pub c: i64,       // challenge: H(data)
}

fn pseudo_rand(seed: u64, idx: usize) -> i64 {
    // xorshift64 — produção: ChaCha20 via rand_chacha
...
/// Gera par de chaves LWE. Produção: semente de OsRng.
...
impl SecretKey {
    /// Fiat-Shamir sobre LWE:
    ///   w = A·y          (commitment)
    ///   c = H(data)      (challenge)
    ///   z = y + c·s mod q (resposta)
    ///
    /// ANTES: retornava Array1::zeros(10) — assinatura sempre nula.
...
/// Verificação: A·z - c·b ≡ w (mod q), com tolerância do erro c·e.
