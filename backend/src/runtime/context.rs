...
impl RequestContext {
    ...
    pub fn with_identity(
        ...
        self.evidence
            .set_identity_verified(identity.verified);

        self.identity = Some(identity);
        self
    }

    pub fn with_policy_id(
        mut self,
...
mod tests {
    ...
    fn identity_updates_runtime_evidence() {
        let context = example_context().with_identity(
            IdentityContext::new(
                "client-001",
                "api_key",
                true,
    ...
    fn request_age_never_underflows() {
        ...
        assert_eq!(
            context.request_age_ms(
                context.timestamp_ms.saturating_sub(1)
            ),
            0
        );
    }
