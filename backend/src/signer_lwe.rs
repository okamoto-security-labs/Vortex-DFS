// signer_lwe.rs - Vortex DFS (CORRIGIDO)
...
// Cargo.toml: adicione `rand = "0.8"` (necessário pro fix do Finding #4)
//
// ============================================================================
// CHANGELOG DE SEGURANÇA (referência: relatório de auditoria adversarial)
// ============================================================================
// FIX Finding #1 (forjamento total): challenge agora é H(w || data), não H(data).
//     O commitment `w` precisa existir ANTES do challenge ser calculado — é
//     isso que impede o forjador de "resolver de trás pra frente".
// FIX Finding #2 (overflow de tolerância): challenge agora é limitado a um
//     intervalo pequeno [-CHALLENGE_BOUND, CHALLENGE_BOUND], e a tolerância
//     é uma CONSTANTE fixa em tempo de compilação — nunca mais derivada de
//     um valor que o atacante/dado influencia. Nunca pode se aproximar de Q/2.
// FIX Finding #4 (reuso de nonce): `nonce` deixou de ser parâmetro público
//     de `sign()`. O commitment y agora é sempre gerado internamente com
//     OsRng — o chamador não tem como escolher nem repetir o nonce.
...
/// da API key dele — era exatamente esse o Finding #3.
